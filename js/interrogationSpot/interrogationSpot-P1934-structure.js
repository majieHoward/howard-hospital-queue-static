...
;(function($){
	...
	$.interrogationSpot = {version: "2017.09.04.P1934"};
	...
	$.interrogationSpot = function (interrogationSpot, options){
		...
		/**add by mayijie 定义tr的class name**/
		var trClassArray=new Array("active","","success","","info","","warning","","danger","");
		/**$.data() 函数用于在指定的元素上存取数据，返回设置值**/
		$.data($interrogationSpot, "interrogationSpot", self);
		
		this.obtainlistOfVisitsBodyData=function(){
			return this.listOfVisitsBody;
		}
		...
		function obtainDiagnosisAreaItems(controller,parameterData, successCallback){
			/**
			*POST //treatment/hospital/queue/obtain.diagnosis.area.items.information.process HTTP/1.1
			**/
		...
		function resizeTable(){
			var windowHeight=screenHeight-pageLogoHeight-3;
			...
			if(listOfVisitsBody.length>0){
				...
				**/
				...
				$.each(listOfVisitsBody,function(identifier,diagnosisAreaItem){
					...
					tr=listOfVisitsBodyTr("",trClassArray[identifier%10],"",{},$("#listOfVisitsBody"));
